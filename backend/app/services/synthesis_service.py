...
class SynthesisService:
    ...
    async def synthesize_strategy(
        ...
        Returns:
            {
                "strategy_type": "SPECULATIVE_REBOUND" | "VALUE_ACCUMULATION" | "RESONANCE_LONG" | "AVOID",
                "title": "策略标题",
                "position_suggest": "仓位建议",
                "action_guide": "操作指导",
                "risk_warning": "风险提示",
                "time_frame": "时间框架",
                "conviction": 1-5,
...
2. strategy_type必须是以下四个之一: SPECULATIVE_REBOUND, VALUE_ACCUMULATION, RESONANCE_LONG, AVOID
3. 标题要一针见血，用中文表达策略本质
...
"""
        ...
        user_prompt = f"请分析以下技术面与基本面的背离情况，给出融合策略：\n{context_text}"

        try:
            # 使用DeepSeek进行深度逻辑整合（如果配置了的话），否则用Qwen
            raw_response = await LLMFactory.fast_reply("deepseek", system_prompt, user_prompt, timeout=30)
            ...
            # 添加原始分析
            result["raw_analysis"] = raw_response[:500] if len(raw_response) > 500 else raw_response
    ...
    def _fallback_strategy(tech_view: Dict, fund_view: Dict) -> Dict[str, Any]:
        ...
        tech_decision = tech_view.get('decision', 'HOLD')
        fund_decision = fund_view.get('decision', 'HOLD')
...
async def synthesize_strategy(
    ...
    """便捷函数：融合技术面和基本面策略"""
    return await SynthesisService.synthesize_strategy(symbol, tech_view, fund_view, context)
